...
import { Plus, Search, Trash2, Edit2, ShieldCheck, Loader2 } from 'lucide-react';
...
export default function TiposCoberturaPage() {
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [searchTerm, setSearchTerm] = useState('');
  const [items, setItems] = useState<TipoCobertura[]>([]);
  const [formData, setFormData] = useState<Partial<TipoCobertura>>({ nome: '' });
...
/*  useEffect(() => {
    const unsub = genericService.subscribe<TipoCobertura>('tipos_cobertura', setItems);
    return () => unsub();
  }, []); */

  useEffect(() => {
    genericService.list<TipoCobertura>('tipos_cobertura').then(setItems);
  }, []);
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (formData.id) {
        genericService.update('tipos_cobertura', formData.id, formData);
        toast.success("Alteração concluída!");
      } else {
        genericService.create('tipos_cobertura', formData);
  ...
  const [itemToDelete, setItemToDelete] = useState<TipoCobertura | null>(null);
  ...
  const handleDelete = async () => {
    ...
    try {
      await genericService.delete('tipos_cobertura', itemToDelete.id);
  ...
  const columns: DataTableColumn<TipoCobertura>[] = [
    {
      header: 'TIPO DE COBERTURA', 
      accessor: (m: TipoCobertura) => <div className="font-semibold text-slate-900">{m.nome}</div>
    },
    {
      header: 'AÇÕES',
      className: "text-right",
      accessor: (m: TipoCobertura) => (
    ...
    <div className="space-y-6 animate-in fade-in duration-700">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 pb-6 border-b border-slate-100">
        <div className="space-y-1">
          <h1 className="text-3xl font-black text-slate-900 tracking-[-0.02em] leading-none uppercase">Tipos de Cobertura</h1>
          <p className="text-xs text-slate-400 font-bold tracking-[0.1em] uppercase opacity-70 mt-2">Classificação de substituições</p>
        </div>
        {isSupervisor && (
          <Button 
      ...
      <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-xl shadow-brand-950/5 overflow-hidden">
        <div className="p-6 border-b border-slate-50 flex items-center justify-between gap-4">
          <div className="relative flex-1 group">
            ...
            <input 
              type="text" 
              placeholder="Pesquisar classificação de cobertura..." 
      ...
      <Modal 
        ...
        title={formData.id ? "Editar Categoria" : "Nova Categoria de Cobertura"}
        size="md"
      >
        <form className="space-y-8" onSubmit={handleSave}>
          <FormField 
            label="Nome da Categoria" 
            placeholder="EX: COBERTURA INTEGRAL" 
            value={formData.nome || ''}
      ...
      >
        <div className="space-y-6">
          <p className="text-sm font-bold text-slate-600">
            Deseja realmente excluir a categoria <span className="text-slate-900 underline">"{itemToDelete?.nome}"</span>?
          </p>
